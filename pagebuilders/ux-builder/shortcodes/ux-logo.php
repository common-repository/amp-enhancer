<?php
// [logo img=""]
function amp_enhancer_ux_logo( $atts, $content = null ){
    ...
    $content = '<div class="'.implode(' ', $classes).'  ux-amp-cont" style="max-width: 100%!important; width: '.$width.'!important"><'.$inner_tag.' class="ux-logo-link block image-'.$hover.'" title="'.$title.'"' . flatsome_parse_target_rel( $link_atts ) . 'href="'.$link.'" style="padding: '.$padding.';"><img src="'.$org_img.'" title="'.$title.'" alt="'.$title.'" class="ux-logo-image block" style="height:'.$height.'px;" /></'.$inner_tag.'></div>';

    return $content;
}
add_shortcode('logo', 'amp_enhancer_ux_logo');
