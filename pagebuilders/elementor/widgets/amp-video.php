...
class Amp_Video extends Widget_Base {
	...
	protected function render() {
		...
		if($settings['video_type'] == 'youtube' ){
			if(isset($settings['youtube_url']) && !empty($settings['youtube_url'])){
				...
				$amp_video_html = '<amp-youtube width="480" id="overlay-'.$this->get_id().'" height="270" layout="'.esc_attr($layout).'" data-videoid="'.esc_attr($youtube_video_id).'" '.$loop.'  '.$autoplay.' '.$controls.' ></amp-youtube>';
			}
			...
			if (preg_match('%^https?:\/\/(?:www\.|player\.)?vimeo.com\/(?:channels\/(?:\w+\/)?|groups\/([^\/]*)\/videos\/|album\/(\d+)\/video\/|video\/|)(\d+)(?:$|\/|\?)(?:[?]?.*)$%im', $settings['vimeo_url'], $regs)){
		            $video_id = $regs[3];
		            $amp_video_html = '<amp-vimeo data-videoid="'.esc_attr($video_id).'" layout="'.esc_attr($layout).'" width="480" height="270" '.$loop.'  '.$autoplay.' '.$controls.'></amp-vimeo>';
			}
			...
			$amp_video_html = '<amp-dailymotion data-videoid="'.esc_attr($video_id).'" layout="'.esc_attr($layout).'" data-ui-highlight="FF4081" width="480" height="270" '.$loop.'  '.$autoplay.' '.$controls.'></amp-dailymotion>';
		}else{
			$amp_video_html = '<amp-video width="480" height="270" src="'.$this->get_hosted_video_url().'" poster="'.$img_src.'" layout="'.esc_attr($layout).'" controls  '.$autoplay.'><div fallback><p>Your browser doesn\'t support HTML5 video.</p>
			    </div>
			    <source type="video/mp4" source="'.$this->get_hosted_video_url().'">
			  </amp-video>';
		}
					...
					if(isset($settings['lightbox']) && $settings['lightbox'] !== 'yes'): ?>
						<div class="amp-overlay-video-player">
						    ...
						    <div id="eleOverlay-<?php echo $this->get_id();?>" class="click-to-play-overlay">
								...
								<?php endif; ?>
								<amp-img alt="<?php echo $alt_txt;?>" tabindex="0" role="button" src="<?php echo esc_url($img_src);?>" placeholder layout="fill" <?php echo $on_tap_lightbox;?>></amp-img>
							</div>
						</div>
					<?php endif;?>
				</div>
	...
	private function render_hosted_video() {
		...
		?>
	<!-- 	<video class="elementor-video" src="<?php //echo esc_url( $video_url ); ?>" <?php //echo \Elementor\Utils::render_html_attributes( $video_params ); ?>></video> -->
		<?php
	}
}
