...
abstract class Theme_Document extends Library_Document {
	...
	public function print_elements_with_wrapper( $elements_data = null ) {
		//print_r($this->get_properties()['location']);die; 
		// Check if the implementing document has optional wrapper tags
		if(method_exists($this, 'get_main_id')){
			$get_main_id = $this->get_main_id();
		}
		if(method_exists($this, 'get_properties')){
			$properties = $this->get_properties();
			$location = isset($properties['location']) ? $properties['location'] : false;
		}
		...
		?>
		<?php if(isset($location) && $location == 'popup'){ ?>
			<div class="dialog-widget dialog-lightbox-widget dialog-type-buttons dialog-type-lightbox elementor-popup-modal" [hidden]="hidePopup" id="elementor-popup-modal-<?php echo esc_attr($get_main_id); ?>"><div class="dialog-widget-content dialog-lightbox-widget-content animated"><div class="dialog-header dialog-lightbox-header"></div><div class="dialog-message dialog-lightbox-message">
         <?php } ?>
		...
		<?php if(isset($location) && $location == 'popup'){ ?>
			</div><div class="dialog-buttons-wrapper dialog-lightbox-buttons-wrapper"></div><div class="dialog-close-button dialog-lightbox-close-button" on="tap:AMP.setState({hidePopup: true})" role="button" tabindex="0" ><i class="eicon-close"></i></div></div></div>
		<?php } ?>
		<?php
	}
